const admin = require('firebase-admin');

const projectId = "digidrobe-backend";
const clientEmail = "firebase-adminsdk-fbsvc@digidrobe-backend.iam.gserviceaccount.com";
const privateKey = "-----BEGIN PRIVATE KEY-----\\\\nrHr0eGcesC9+UoGOlR2c1R49\n-----END PRIVATE KEY-----\n".replace(/\\n/g, '\n');

if (admin.apps.length === 0) {
    admin.initializeApp({
      credential: admin.credential.cert({
        projectId,
        clientEmail,
        privateKey,
      }),
    });
}

const db = admin.firestore();

async function runTestMigration() {
  const testUserId = "TEST_CLAIM_USER";
  console.log(`--- Migrating anonymous items to ${testUserId} ---`);
  
  const snapshot = await db.collection('wardrobeItems')
    .where('userId', '==', 'anonymous')
    .get();

  if (snapshot.empty) {
    console.log('No anonymous items to migrate.');
    return;
  }

  console.log(`Found ${snapshot.size} items. Proceeding with migration...`);
  const batch = db.batch();
  snapshot.docs.forEach(doc => {
    batch.update(doc.ref, { userId: testUserId, updatedAt: new Date().toISOString() });
  });

  await batch.commit();
  console.log('Migration successful!');

  console.log('Checking result...');
  const check = await db.collection('wardrobeItems').where('userId', '==', testUserId).get();
  console.log(`User ${testUserId} now has ${check.size} items.`);
}

runTestMigration().catch(console.error);
